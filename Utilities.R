...
egg_dev<-function(date, lng, lat, days.min, days.max){
  dev_day<-NA
  sp <- SpatialPoints(cbind(lng, lat))
  ...
  if(!is.na(dev_day)){
    if (as.numeric(dev_day-date)<days.min) dev_day<-NA
    if (as.numeric(dev_day-date)>days.max) dev_day<-NA
  }
  return(dev_day)
}
...
hopper_dev_dur<-function(date, lng, lat, days.min, days.max){
  dev_day<-NA
  sp <- SpatialPoints(cbind(lng, lat))
  whD<-max(which(datesMET<=date))
  fln<-paste0("data/Met_Data_", str_c(unlist(strsplit(as.character(datesMET[whD]),"-")),
...
hopper_dev<-function(date, lng, lat, thresh, days.min, days.max, peak=TRUE, lambda=1, m=2){
  ans<-NA
  dev.day<-hopper_dev_dur(date, lng, lat, days.min, days.max)
  if(!is.na(dev.day)){
...
swrm_stay_type<-function(lct, ndvi, npsi=6){
  ans<-"tba"
  vi.lct<-get_lct(lct)
  if(!vi.lct=="Other"){
    vi.tr<-get_NDVI_trend(ndvi, npsi)
...
get_direction_v2<-function(x0,x1){
  P1<-c(x1$lng-x0$lng, x1$lat-x0$lat)
  P2<-abs(c(x1$lng-x0$lng,0))
  if(P1[2]>=0){ # 1&2 quadrant
    a<-as.numeric(angle(P1, P2)) 
  } else {
    a<-360-as.numeric(angle(P1, P2)) 
  }
  return(a)
}

get_direction_all<-function(wh, date){
  a<-c()
  if(wh<10) s<-paste0("0000",wh)
  if(wh>=10 & wh<100) s<-paste0("000",wh)
  if(wh>=100 & wh<1000) s<-paste0("00",wh)
  ...
  yy<-ncvar_get(dat, "latitude")
  nc_close(dat)
  nn<-ncol(xx)   #  Number of time points
  for(k in 1:1000){
    x0<-data.frame(lng=src$lng[wh], lat=src$lat[wh])
    x1<-data.frame(lng=xx[k,nn], lat=yy[k,nn])
    a<-c(a, get_direction_v2(x0,x1))
  }
  return(data.frame(date=date, ind=wh, sample=1:1000, angle=a))
}
